...
var _ = newState(solver.LevelEasy, nil, nil)

func newState(level solver.Level, sol *boards.Solution, r *Random) *state {
	if r == nil {
		r = NewRandom()
	}
	if sol == nil {
		sol = GenerateSolution(r)
	}
	b := sol.Clone(boards.Edit)

	// Mark all cells as read-only.
	for i := range boards.Size {
		b.SetReadOnly(i, b.Get(i))
	}
...
func (s *state) getLastResultOrSolve() *solver.Result {
	if s.lastResult != nil {
		return s.lastResult
	}
	return s.solve()
...
func shouldContinueByLevel(desiredLevel solver.Level, r *random) bool {
	switch desiredLevel {
