...
func Equivalent(b1, b2 BoardBase) bool {
...
func EquivalentReadOnly(b1, b2 BoardBase) bool {
	for i := range BoardSize {
...
func ContainsAll(b1, b2 BoardBase) bool {
	for i := range BoardSize {
		v := b2.Get(i)
		if v != 0 && b1.Get(i) != v {
			return false
...
func ContainsReadOnly(b1, b2 BoardBase) bool {
	for i := range BoardSize {
